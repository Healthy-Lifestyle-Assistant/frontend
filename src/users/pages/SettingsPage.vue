<template>
  ...
  <div class="d-flex flex-column align-items-center">
    ...
    <form
      v-if="!isDeleted"
      @submit.prevent="submitFormDelete"
      style="min-width: 13rem; width: fit-content"
      class="mb-5"
    >
...
export default {
  ...
  async created() {
    this.$store.commit("setCurrentUrl", `/settings/${this.$route.params.id}`);
  ...
  methods: {
    async submitForm() {
      const requestDto = {
        updatedUsername:
          this.username == this.oldUsername ? null : this.username,
        updatedEmail: this.email == this.oldEmail ? null : this.email,
        updatedFullName:
          this.fullName == this.oldFullName ? null : this.fullName,
        updatedPassword:
          this.password == this.oldPassword ? null : this.password,
        updatedConfirmPassword:
          this.password == this.oldConfirmPassword
            ? null
            : this.confirmPassword,
        updatedCountryId:
          this.countryId == this.oldCountryId ? null : this.countryId,
