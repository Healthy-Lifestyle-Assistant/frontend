<template>
    <div class="card mb-3 me-3 mental-card-width">
        <div class="card-body">
            ...
            <div v-if="bodyParts" class="card-text">
                <span v-for="bodyPart in bodyParts" :key="bodyPart.id" class="body-parts-span">
                    <small class="body-parts">{{ bodyPart.name.toLowerCase() }}</small>&nbsp;</span>
            </div>

            <div v-if="isCustom" class="d-flex justify-content-end mt-3 me-4">
                <router-link :to="generateLinkManage()" class="btn btn-outline-secondary d-block me-3">Manage
                </router-link>
...
export default {
    ...
    methods: {
        generateLinkDetails() {
            return `/meditations-details/${this.isCustom ? 'custom' : 'default'}/${this.id}`;
        },

        generateLinkManage() {
            return `/meditations-manage/${this.id}`;
        },
    },
